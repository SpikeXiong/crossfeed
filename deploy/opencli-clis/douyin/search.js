...
export function projectCard(card, index) {
    ...
    for (let i = 0; i < texts.length; i++) {
        ...
        if (!published && /^(刚刚|今天|昨天|前天)$/.test(t)) { published = t; continue; }
        if (!published && /^\d+\s*(秒|分钟|小时|天|周|个月|月|年)前$/.test(t)) { published = t; continue; }
        if (!published && /^\d{4}[-/.年]\d{1,2}(?:[-/.月]\d{1,2}日?)?$/.test(t)) { published = t; continue; }
...
export const WAIT_AND_EXTRACT_JS = (timeoutMs) => `
  new Promise((resolve) => {
    const collectCards = () => {
      const cards = [];
      ...
      for (const li of lis) {
        ...
        const cover = (img && (img.currentSrc || img.src || img.getAttribute('data-src')))
          || (video && video.poster)
          || '';
        cards.push({ href: a.getAttribute('href') || '', leafTexts, cover });
      }
      return cards;
    };
    const detectState = () => {

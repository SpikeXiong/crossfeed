...
import { apiGet, stripHtml } from '../../node_modules/@jackwener/opencli/clis/bilibili/utils.js';
cli({
    ...
    columns: ['rank', 'title', 'author', 'score', 'url', 'pic', 'bvid', 'play', 'pubdate'],
    func: async (page, kwargs) => {
        ...
        return results.slice(0, Number(limit)).map((item, i) => {
            if (searchType === 'bili_user') {
                return { rank: i + 1, title: stripHtml(item.uname ?? ''), author: (item.usign ?? '').trim(), score: item.fans ?? 0, url: item.mid ? `https://space.bilibili.com/${item.mid}` : '', pic: item.upic ?? '', bvid: '', play: item.fans ?? 0, pubdate: '' };
            }
            const pic = item.pic ? (String(item.pic).startsWith('http') ? item.pic : `https:${item.pic}`) : '';
            return {
                rank: i + 1,

    ...
    return {
      items: feeds.map(entry => {
        ...
        return {
          ...
          // Cover is already on the hydrated card. Crossfeed used to call
          // note-cover (one full page load per item) which trips 频繁请求.
          cover: card.cover?.urlDefault ?? card.cover?.urlPre ?? card.cover?.url ?? '',
...
/** XHS note ids are Mongo ObjectIDs; first 8 hex chars are unix seconds. */
function noteIdToIso(id) {
    const hex = String(id || '').trim().slice(0, 8);
...
export async function runFeed(page, kwargs, webHost) {
    ...
    for (const row of data.items) {
        ...
        rows.push({
            ...
            cover: toCleanString(row.cover),
            url: buildFeedNoteUrl(webHost, id, xsecToken),
            published_at: noteIdToIso(id),
        });
    }
    if (rows.length === 0) {
...
export const command = cli({
    ...
    columns: ['id', 'title', 'author', 'likes', 'type', 'url', 'cover', 'published_at'],
